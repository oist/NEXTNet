	...
	:adjacencylist_network(true, true)
{
    ...
    for (node_t u = 0; u < size; u++) {
        ...
        for (node_t v : vs) {
            ...
            const bool s1  = u_neighbours.erase(v);
            assert(s1);
            _unused(s1);
            const bool s2 = nodes_neighbours[v].erase(u);
            assert(s2);
            _unused(s2);
            const bool s3 = u_neighbours.insert(w).second;
            assert(s3);
            _unused(s3);
            const bool s4 = nodes_neighbours[w].insert(u).second;
            assert(s4);
            _unused(s4);
        }
...
std::vector<int> config_model_clustered_serrano::triangles_binomial(std::function<double(int)> ck, std::vector<int> degrees, rng_t &engine)
{
...
config_model_clustered_serrano::config_model_clustered_serrano(std::vector<int> degrees, double alpha, double beta, rng_t &engine)
    : config_model_clustered_serrano(degrees, [alpha](int k) -> double { return 0.5 * std::pow((double)(k - 1), -alpha); }, beta, engine){};

config_model_clustered_serrano::config_model_clustered_serrano(std::vector<int> degrees, std::function<double(int)> ck, double beta, rng_t &engine)
