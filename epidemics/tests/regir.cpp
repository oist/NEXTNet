...
#if ENABLE_PLOTTING
TEST_CASE("Plot SIS single trajectory (REGIR)", "[REGIR]") {
	using namespace std::string_literals;
	std::mt19937 engine;
	...
	average_trajectories(engine, [&](rng_t& engine){
		...
		env.simulator.reset(new simulate_regir(*env.nw, psi, &rho));
		return env;
	}, t_sim, y_sim_new, y_sim_total, T);

	plot("regir.sis.single.pdf", "SIS single trajectory [REGIR]", [&](auto& gp, auto& p) {
